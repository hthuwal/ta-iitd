...
let check_zero x = if (float_zero x) then 0.0 else x;;
...
let string_of_float_2 f = valid_float_lexem (format_float "%.10g" f);;

let apply l = List.map string_of_float_2 (List.map check_zero l);;
