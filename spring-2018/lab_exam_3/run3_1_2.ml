...
let print_error value col student ta = 
    print_string "INCORRECT ANSWER: \n"; 
    Printf.printf "Value: %d, Col: %d\n" value col;
...
let runner sudoku_stud sudoku_ta value col = 
    let student = Test.eliminateValueCol sudoku_stud value col in
    let ta = Model.eliminateValueCol sudoku_ta value col in
    if (student = ta) && (Model.compareSudokus sudoku_stud sudoku_ta) then (gpt)
    else (print_error value col student ta;0.0);;
...
let () =
  ...
  fprintf oc "%f\n" (runner sudoku_student sudoku_teacher value col);   (* write something *)   
  close_out oc;;
