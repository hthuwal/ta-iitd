...
evaluate()
{
    ...
    if [ $status == "OK" ]; then
        part="a"
        run "$t1" "$logs" "$fname" "$part" "$data_folder_path/devnagri_train.csv" "$data_folder_path/devnagri_test.csv" "$stud_folder_path/predictions_neural_${part}_1" 128 0.01 "sigmoid" 
        ...
        run "$t1" "$logs" "$fname" "$part" "$data_folder_path/devnagri_train.csv" "$data_folder_path/devnagri_test.csv" "$stud_folder_path/predictions_neural_${part}_2" 64 0.01 "relu" 100
        ...
        run "$t1" "$logs" "$fname" "$part" "$data_folder_path/devnagri_train.csv" "$data_folder_path/devnagri_test.csv" "$stud_folder_path/predictions_neural_${part}_3" 64 0.1 "tanh" 10 10
